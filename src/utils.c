...
// isTry is a flag to denote try to move or excute a move
...
/**
 * copy board form boardA to baordB
 */
void copy_board(uint8_t boardA[SIZE][SIZE], uint8_t boardB[SIZE][SIZE]){
    uint8_t x, y;
    for(x = 0; x < SIZE; x++){
        for(y = 0; y < SIZE; y++){
            boardB[x][y] = boardA[x][y];
        }
    }
}

/**
 * judge whether two board is same or not
 */
